...
import { BASELINE, GRID, MUTED, PROFIT, REVENUE, SURFACE, TEXT_SECONDARY, compact } from './tokens'
    ...
    <div className="chart-tip">
      <TipBody
        ...
        rows={[
          ...
          { color: PROFIT, label: 'Profit', value: money(p.profit) },
          { color: MUTED, label: `Cost · ${p.orders} paid orders`, value: money(p.cost) },
        ]}
      />
    </div>
  )
...
export default function TrendChart({ trend }) {
  ...
  const empty = points.length === 0 || points.every((p) => Number(p.revenue) === 0 && Number(p.cost) === 0)
  const title = granularity === 'month' ? 'Revenue and profit by month' : 'Revenue and profit by day'
  ...
  const line = (dataKey, color, text) => (
    <Line
      ...
      label={labelsFit ? (props) => <EndLabel {...props} last={last} text={text} /> : false}
    ...
    <div className="viz-card">
      <div className="viz-head">
        <div>
          ...
          <p className="muted viz-sub">Paid orders</p>
        </div>
      ...
      {empty ? (
        ...
        <>
          <ul className="legend" aria-label="Legend">
            ...
            <li><span className="key-line" style={{ background: PROFIT }} />Profit</li>
          </ul>
          ...
          <table>
            <thead>
              <tr><th>{granularity === 'month' ? 'Month' : 'Day'}</th><th className="num">Paid orders</th>
                <th className="num">Revenue</th><th className="num">Cost</th><th className="num">Profit</th></tr>
            </thead>
            <tbody>
              {points.map((p) => (
                <tr key={p.period}>
